...
impl LatLon {
    ...
    // LatLon::parse()
    fn parse(input: &str) -> Result<LatLon, String> {
        // orig=30.1& wrong format
        // let split = input.split(',').collect::<Vec<_>>();

        // if split.len() != 3 {
        //    return Err("wrong format for latlon".to_string());
        // }
        // add lat,lon,  add this field altitude/elevation

        // best practice.

        // ? is shorthand for early return. syntax sugar
        // let lat = split[0].parse().map_err(|e|format!("{}", e))?;
        // let long = split[1].parse().map_err(|e|format!("{}", e))?;
